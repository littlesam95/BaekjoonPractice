...
void init() { // 명령어를 저장하는 vector 초기화
	Flag = true;
	Query.clear();
}
	...
	for (int i = 0; i < Query.size(); i++) {
		...
		if (cmd.substr(0, 3) == "NUM") {
			...
			if (Stack.size() >= 2) {
				...
				if (A != 0) { // 제수가 0이면 에러
					if (A > 0) {
						if (B >= 0) { // 둘다 양수인 경우 몫은 양수
							Stack.push_back(B / A);
						}
						else { // 피제수가 음수인 경우 몫은 음수
							A = abs(A);
							B = abs(B);
							LL R = B / A;
							Stack.push_back(-R);
						...
						if (B >= 0) { // 제수가 음수인 경우 몫은 음수
							A = abs(A);
							B = abs(B);
							LL R = B / A;
							Stack.push_back(-R);
						}
						else { // 둘다 음수인 경우 몫은 양수
							A = abs(A);
							B = abs(B);
							LL R = B / A;
							Stack.push_back(R);
			...
			if (Stack.size() >= 2) {
				...
				Stack.erase(Stack.begin() + (size - 2));
				if (A != 0) { // 제수가 0이면 에러
					if (B >= 0) { // 피제수가 양수면 나머지는 양수
						A = abs(A);
						B = abs(B);
						LL R = B % A;
						Stack.push_back(R);
					}
					else { // 피제수가 음수면 나머지는 음수
						A = abs(A);
						B = abs(B);
						LL R = B % A;
						Stack.push_back(-R);
