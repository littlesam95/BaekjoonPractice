...
void Mal_Move(int I) {
	...
	if ((nextY >= 1) && (nextY <= N) && (nextX >= 1) && (nextX <= N)) { // 이동할 칸이 격자를 벗어나지 않는 경우
		if (MAP[nextY][nextX] == 0) { // 다음 칸이 흰색인 경우 현재 칸에 있는 모든 말들을 다음 칸으로 이동시킨다.
		...
		else if (MAP[nextY][nextX] == 1) { // 다음 칸이 빨간색일 경우 현재 칸에 있는 모든 말들을 순서를 거꾸로 해서 다음 칸으로 이동시킨다.
...
int main() {
	...
	while (1) {
		...
		for (int i = 1; i <= K; i++) {
			if (MAP_Mal[Mal[i].Y][Mal[i].X][0] == i) { // 말판이 칸의 맨 밑에 있을 경우에만 움직인다.
				Mal_Move(i);
			}
